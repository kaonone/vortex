...
def test_staking(deployer, mcb, test_strategy_deposited, governance, mcbStaking):
    ...
    test_strategy_deposited.stake({"from": governance})
    assert mcbStaking.balanceOf(test_strategy_deposited) == 5000e18
    with brownie.reverts():
        test_strategy_deposited.withdrawMCB({"from": governance})
    test_strategy_deposited.restake({"from": governance})
    with brownie.reverts():
        test_strategy_deposited.withdrawMCB({"from": governance})
    ...
    with brownie.reverts("!nothing to restake"):
        test_strategy_deposited.restake({"from": governance})
