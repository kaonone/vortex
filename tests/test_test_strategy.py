...
def test_open_perp_position(test_strategy, deployer, accounts, governance, token):
    ...
    assert tx.events["PerpPositionOpened"]["perpPositions"] == test_strategy.getMarginPositions()
...
def test_close_perp_position(test_strategy, deployer, accounts, governance, token):
    ...
    assert abs(tx.events["PerpPositionClosed"]["collateral"] - test_strategy.getMarginCash() / 1e12) <= constants.ACCURACY
